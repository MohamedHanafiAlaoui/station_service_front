...
export class Approvisionnements implements OnInit {
  ...
  loadApprovisionnements(stationId: number): void {
    ...
    this.approvisionnementService.getApprovisionnementsByStation(stationId).subscribe({
      next: (data) => {
        this.approvisionnements = this.mapApprovisionnements(data).reverse(); // Newest first
        this.currentPage = 1;
  ...
  private mapApprovisionnements(apps: any[]): any[] {
    return apps.map(app => ({
      ...app,
      dateFormatted: new Date(app.dateApprovisionnement).toLocaleString(),
      pompeInfo: app.pompe ? `P${app.pompe.codePompe}` : 'N/A',
